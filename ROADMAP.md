...
## 🧠 Tutor adaptativo (completo, las 6 fases) — plan completo en `C:\Users\nicoi\.claude\plans\me-gustar-a-que-la-hazy-moth.md`
...
- **Fase 3 — Onboarding needs-analysis** ✅: 4 pantallas nuevas — "¿Para qué querés el inglés?" (9 tareas reales tocables + texto libre), "¿Cuánto tiempo tenés?" (minutos/día + fecha límite opcional → activa espaciado expansivo), afectivo (3 caras sin números: vergüenza al hablar, cuánta corrección, cuánto español de apoyo), y la prescripción "Así vamos a trabajar" (mezcla de práctica, modo de espaciado, temas a reforzar, todo explicado en palabras simples). La preferencia de formato (existía antes) se mantiene como desempate, nunca filtra actividades. Test de nivel ahora es adaptivo: corta después de 2 fallos seguidos en vez de forzar las 10 preguntas siempre. Camino de migración para usuarias existentes ("Unas preguntas nuevas, un minuto") que llena el perfil sin rehacer el test de nivel. Verificado de punta a punta con clics reales: perfil ansioso da menos % de hablar y más repaso; perfil tranquilo da la mezcla estándar; migración salta el test correctamente.
- **Fase 4 — Recomendador en el inicio** ✅: el inicio ahora abre con "Hoy te conviene…" (3 tarjetas con el motivo explicado) en vez de un menú fijo; el menú completo queda colapsado debajo con un botón "Ver todas las actividades". Puntaje = 40% necesidad (tarjetas vencidas, errores, dominio débil) + 25% qué tan bien encaja con la prescripción + 20% variedad (evita repetir lo mismo muchos días seguidos) + 15% cuánto lo usa. Gates duros: sin cuota de la profe o sin clave configurada, solo recomienda actividades gratis; con mucha vergüenza al hablar y menos de 3 días de uso, la llamada no aparece. Verificado con 5 estados sintéticos (recién empieza, muchas tarjetas vencidas, ansiosa, sin cuota, sin clave): las 5 dan recomendaciones distintas y con el gate correcto en cada caso (confirmado que "llamada" desaparece con ansiedad alta, y que solo quedan actividades gratis sin cuota/clave).
- **Fase 5 — Coach con IA** ✅: al terminar una clase/lección/llamada (con la llamada, solo si tuvo 6+ turnos), 1 sola llamada a la IA revisa el desempeño real y sugiere ajustes chicos a la mezcla de práctica y a los temas de foco — nunca al dominio medido (eso lo escribe solo el motor local, la IA no puede tocarlo). Cada campo del ajuste se valida por separado: la mezcla debe sumar 1 y estar entre 0-1, los temas deben existir en el catálogo real, máximo 3 cambios por semana, máximo 1 revisión cada 6 horas y solo si sobra cuota de la profe. Todo ajuste rechazado queda registrado, y hay botón para deshacer el último. Vista "Cómo te veo" muestra el motivo del último ajuste. Verificado inyectando 10 patches maliciosos/malformados (dominio falso, valores absurdos, tema inventado, HTML con script, JSON roto, null): ninguno corrompió el estado ni el dominio medido; el intento de XSS se mostró como texto, no se ejecutó. Verificado también que una sesión real hace exactamente 1 llamada extra del coach, y que repetir el cierre de sesión enseguida no dispara una segunda (límite de 6 horas).
- **Fase 6 — Contenido real** ✅: catálogo curado a mano de 35 ítems **reales y verificados** (series, películas, canciones, podcasts, libros/lecturas graduadas — Peppa Pig, Friends, The Office, Oxford Bookworms, BBC Learning English, etc.), con al menos 5 por nivel A1-C1. La IA no interviene en la lista — solo se recomienda lo que está en el catálogo, rankeado por nivel e intereses reales del onboarding. Si algo "no se encuentra", se descarta para siempre; "trabajarla" la manda directo al material de la profe. Nunca afirma en qué plataforma está (evita inventar dónde ver algo). Nota honesta: preferí un catálogo más chico pero 100% verificado (35 ítems) en vez de forzar 60 con riesgo de inventar algún título. Verificado: nivel A1 + interés familia da 3 sugerencias reales del catálogo, todas A1; "no la encontré" la excluye para siempre; ninguna respuesta nombra Netflix/Spotify/etc.
